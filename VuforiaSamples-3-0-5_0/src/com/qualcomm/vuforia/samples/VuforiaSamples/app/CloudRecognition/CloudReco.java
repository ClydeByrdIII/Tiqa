...
import com.qualcomm.vuforia.samples.VuforiaSamples.LanguageActivity;
...
public class CloudReco extends Activity implements SampleApplicationControl, SampleAppMenuInterface {
	...
	private static final String kAccessKey = "fe69ca042fef3c389e03988d52a8ba008af8bd0b";
	private static final String kSecretKey = "cee594ed9221f2bbf63fe068c41bcf27936406b2";
		...
		runOnUiThread(new Runnable() {
			public void run() {
				...
				builder.setMessage(getStatusDescString(CloudReco.this.mlastErrorCode))
						.setTitle(getStatusTitleString(CloudReco.this.mlastErrorCode)).setCancelable(false).setIcon(0)
	...
	public void onQCARUpdate(State state) {
		...
		if (statusCode < 0) {
			...
			if (finder.getResultCount() > 0) {
				...
				showToast(result.getTargetName());
				Intent intent = new Intent(this, LanguageActivity.class);
				intent.putExtra("word", result.getTargetName());
	...
	private class SleepRunnable implements Runnable {
		...
		public void run() {
			try {
				Looper.prepare();
				Thread.sleep(5000);
				showToast("Image not found in database. Going back to main menu.");
